...
import { createExecFileOps, createExecExport, posixJoin } from "./exec-file-ops.ts";
...
export function createKubernetesSandbox(workspace: WorkspaceStore, opts: KubernetesSandboxOptions): Sandbox {
  ...
  const sandboxName = (kind: "resident" | "scratch", key: string) =>
    `qm-k8s-${hash(JSON.stringify([opts.orgId, kind, key]))}`;
  ...
  const sandbox: Sandbox = {
    ...
    async provision(layers, options) {
      ...
      const kind = options?.scratch ? "scratch" : "resident";
      const key = options?.scratch?.key ?? scope;
      const identity = JSON.stringify([opts.orgId, kind, key]);
      const name = sandboxName(kind, key);
    ...
    async destroyScope(scopeId) {
      const name = sandboxName("resident", scopeId);
      await queue(name, async () => {
        await deleteResource("Pod", name);
        await deleteResource("PersistentVolumeClaim", name);
        await deleteResource("NetworkPolicy", name);
        active.delete(name);
      });
    },
