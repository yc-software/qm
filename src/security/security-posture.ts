...
export function securityScreenPayload(input: SecurityScreenInput): SecurityScreenPayload | null {
  ...
  if (
    ...
    if (content.trim()) payloads.push({ source: input.verifiedSwarm ? "swarm-delegation" : input.surface, content });
  }
