...
export function renderSharingPosturePrompt(actor: Principal, sourceScopes: readonly ScopeId[], admin = false): string {
  if (sourceScopes.length === 0) return "";
  if (admin)
    return `## Context sources\nAlready loaded sources are labelled by origin: ${sourceScopes.join(", ")}. This is not an administrative access allowlist. Use administrative discovery for other resources needed by the request; do not expose unrelated private information.`;
