...
const schema = z.strictObject({
  ...
  authParams: z
    ...
    .optional(),
  tenants: z
    .strictObject({ url: httpsUrl, idField: field, labelField: field, itemsField: field.optional() })
    .optional(),
});
...
export function oauthProvidersFor(
  ...
  for (const connector of connectors) {
    ...
    if (connector.tenants && connector.hosts.length !== 1)
      throw new Error("tenant connectors require one canonical API host");
    if (connector.tenants && !connector.hosts.includes(new URL(connector.tenants.url).hostname))
      throw new Error("tenant URL must belong to a declared API host");
