...
import { downscaleVisionImage, sniffImageDimensions } from "../core/image-downscale.ts";
  ...
  return {
    ...
    async read(path: string, signal?: AbortSignal): Promise<ReadResult> {
      ...
      if (sharedFile) {
        ...
        signal?.throwIfAborted();
        return {
          ...(await withAbort(() => fileReadResult(materializedPath, bytes, granted.ownerScopeId), signal)),
          content:
      ...
      return timed("file_op", async () => {
        ...
        if (direct !== null) return withAbort(() => fileReadResult(path, direct, writableScopeId), signal);
        for (const mount of fallbackMounts) {
          ...
          if (v !== null) return withAbort(() => fileReadResult(path, v, mount.scopeId), signal);
        }
...
async function fileReadResult(path: string, bytes: Uint8Array, sourceScopeId: ScopeId | null): Promise<ReadResult> {
