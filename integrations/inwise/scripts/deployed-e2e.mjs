...
if (!qmRepo)
  throw new Error("Set QM_REPO to a current yc-software/qm checkout");
...
const relayState = join(temporary, "relay.json");
const children = [];
...
function runQm(args, options = {}) {
  if (process.platform === "win32" && wslNode) {
    ...
    const translated = args.map((value) =>
      value === qmRepo ? linuxPath(value) : value,
    );
    return run(
      "wsl.exe",
      [
...
try {
  if (process.platform === "win32" && !wslNode) {
    throw new Error(
      "QM's Docker target requires a POSIX runtime; set QM_WSL_NODE to a Node 24 binary in WSL",
    );
  }
  ...
  if (existingContainers || existingVolumes) {
    throw new Error(
      "Refusing to reuse existing qm-inwise-qm-e2e Docker resources",
    );
  }
  runQm(["check"]);
  ...
  const runtime = JSON.parse(
    (await readFile(sourceConfig, "utf8")).replace(/,\s*([}\]])/g, "$1"),
  );
  ...
  startNode(relayEntry, [], {
    ...
    INWISE_QM_STATE_FILE: relayState,
  });
  await waitFor(`${relayForHost}/healthz`, "Inwise QM relay");

  const login = runInSandbox(
    ["auth", "login", "--relay", relayForSandbox],
    true,
  );
  ...
  const verificationCode = paired.match(
    /Verification code:\s*([A-F0-9-]+)/,
  )?.[1];
  if (!verificationCode)
    throw new Error("Laptop edge did not return a verification code");
  ...
  runInSandbox(
    [
      ...
      "search_meetings",
      "--json",
      '{"query":"__inwise_qm_deployed_e2e_no_match__","limit":1}',
    ],
    true,
  );
