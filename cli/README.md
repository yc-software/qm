...
infra/
.github/workflows/qm-update.yml
```
...
resolves the same interpreter; upgrade the pin deliberately. Fly and AWS scaffolds also include
the GitHub Actions runner used by Admin's browser updater.
...
`update` selects only stable releases that have passed the repository's seven-day dependency
cooldown. `--version` freezes an already-eligible release for the browser workflow; a newer release
appearing while that workflow runs does not change its target.
