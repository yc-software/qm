    ...
    JSON.stringify(
      {
        ...
        sandbox: opts.sandbox as "local" | "sprites" | "smolmachines" | "boxd" | "auto",
        canaryChannel,
        strict: opts.strict,
        slack: withSlack,
      },
...
async function main(): Promise<number> {
  ...
  switch (command) {
    ...
    default:
      console.error(
        "usage: dev [up|down|status|restart|canary|logs|doctor] [--json] [--force] [--rotate] [--strict] [--sandbox local|sprites|smolmachines|boxd|auto] [--no-slack] [--no-watch] [--org id] [--fix]",
      );
      return EXIT.usage;
  }
}
