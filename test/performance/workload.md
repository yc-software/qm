...
Programmatic callers may pass an optional `AbortSignal` to `runWorkload`. Cancellation stops new arrivals, cancels active requests and streams, and retains the counts already offered and completed. The summary reports `cancelledAt`, actual `finishedAt`, the original `plannedFinishAt`, and `measurementComplete:false`; `pass` is always false. Rates use the actual elapsed interval while the target rate retains the planned trace rate. A caller that dispatches native application work must also stop its owned runs and schedules and retain separate cleanup receipts; cancelling the HTTP request alone does not undo accepted work.
