...
test("destroyScope removes resident resources without provisioning", async () => {
  const f = fixture();
  const h = await f.sandbox.provision(layers);
  await f.sandbox.teardown(h, { keepWarm: true });
  assert.equal(f.objects.size, 3);
  await f.sandbox.destroyScope!(layers[0]!.scopeId);
  assert.equal(f.objects.size, 0);
  await f.sandbox.destroyScope!(layers[0]!.scopeId);
});

test("configured Kubernetes backend joins the upstream backend registry", () => {
  assert.deepEqual(enabledSandboxBackends(loadConfig({})), ["local"]);
  const config = loadConfig({ KUBERNETES_SANDBOX_IMAGE: "image:v1" });
  assert.ok(enabledSandboxBackends(config).includes("kubernetes"));
  assert.equal(fixture().sandbox.exportFiles instanceof Function, true);
  assert.equal(fixture().sandbox.importFiles instanceof Function, true);
});
