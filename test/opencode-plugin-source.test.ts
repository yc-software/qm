...
test("OpenCode prompt disables bridged tools absent from this turn", () => {
  ...
  assert.match(harnessSource, /for \(const tool of tools\) enabled\[bridgeTurnToolName\(tool\.name, turn\)\] = true/);
  assert.match(harnessSource, /\[bridgeToolName\("slack"\)\]: false/);
  assert.match(
    harnessSource,
