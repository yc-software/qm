...
test("per-user calls resolve only the caller's fresh token while discovery uses catalog auth", async (t) => {
  ...
  const service = createMcpToolService({
    ...
    fetchImpl: async (_url, init) => {
      ...
      if (rpc.method === "initialize") return jsonResponse({ result: {} });
...
test("mcp client surfaces initialize auth and server failures instead of falling back", async () => {
  for (const status of [401, 403, 500]) {
    const fetch: McpFetch = async () => jsonResponse({ error: "initialize failed" }, status);
    const client = createMcpClient({ url: "https://mcp.example.com/mcp", auth: { mode: "none" }, fetchImpl: fetch });
    await assert.rejects(() => client.listTools(), new RegExp(`initialize failed \\(HTTP ${status}\\)`));
  }
});
...
test("an expired session retries a tool call once on its replacement", async () => {
  let issued = 0;
  let toolCalls = 0;
  const fetch: McpFetch = async (_url, init) => {
    const req = JSON.parse(init.body) as { id?: number; method: string };
    ...
    if (req.method === "tools/call") {
      toolCalls += 1;
      if (init.headers["mcp-session-id"] === "sess-1") return jsonResponse({ error: "gone" }, 404);
      return jsonResponse({ jsonrpc: "2.0", id: req.id, result: { content: [{ type: "text", text: "updated" }] } });
    }
    return jsonResponse({ jsonrpc: "2.0", id: req.id, result: { tools: TOOLS } });
  };
  const client = createMcpClient({ url: "https://mcp.example.com/mcp", auth: { mode: "none" }, fetchImpl: fetch });
  assert.equal(mcpResultText(await client.callTool("update", {})), "updated");
