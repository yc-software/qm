...
test("exposure is provider-key-aware: a model whose provider is unconfigured is not serviceable", () => {
  const noOpenai = { anthropic: true, openai: false, openrouter: false, xai: false };
...
test("provider-key gating applies only to key-authed harnesses (no over-hiding on CLI-auth harnesses)", () => {
  const noKeys = { anthropic: false, openai: false, openrouter: false, xai: false };
  ...
  assert.deepEqual(
    modelProviderAvailabilityFor("pi", noKeys, { anthropic: false, openai: true, openrouter: true, xai: true }),
    { anthropic: false, openai: true, openrouter: true, xai: true },
  );
  assert.deepEqual(
    modelProviderAvailabilityFor(
      "opencode",
      { anthropic: true, openai: true, openrouter: true, xai: true },
      { anthropic: false, openai: false, openrouter: false, xai: false },
    ),
    { anthropic: true, openai: true, openrouter: false, xai: true },
  );
  assert.deepEqual(modelProviderAvailabilityFor("codex", noKeys), noKeys);
  assert.deepEqual(
    modelProviderAvailabilityFor("codex", { anthropic: false, openai: true, openrouter: false, xai: false }),
    {
      anthropic: false,
      openai: true,
      openrouter: false,
  ...
  assert.deepEqual(modelProviderAvailabilityFor("grok", noKeys), noKeys);
