...
Direct uploads are enabled when the deployment uses S3 file storage and Postgres. Upload sessions reside in `file_uploads`; process-local state is not required for recovery. The worker sweeper retries expired sessions each minute and aborts old multipart uploads under the Files upload prefix, including uploads abandoned before their session could be recorded. It does not change bucket lifecycle configuration. Deployments should also configure S3 abort-incomplete-multipart lifecycle rules as protection when core is unavailable for an extended period.
