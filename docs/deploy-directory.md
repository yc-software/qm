...
Core stores share one query pool per database connection string and TLS configuration in each process. `PG_QUERY_POOL_MAX` caps that pool (default **8**); `PG_SESSION_POOL_MAX` separately caps long-held LISTEN and advisory-lock connections (default **8**). Both budgets must be positive integers. When the session budget is at least five, four connections are reserved for LISTEN and leader coordination, leaving the remainder for operation locks. Budgets of one through four retain a single shared session pool and log a warning instead of rejecting startup or increasing the configured budget; coordination isolation is unavailable in this compatibility mode. With the defaults, this is four coordination connections plus four operation connections, not eight plus four. Increasing the session budget above eight adds operation capacity without growing the coordination reserve. These are non-secret core environment settings. Closing a store releases its ownership; the last owner closes the underlying pool. Acquiring a connection times out after five seconds rather than waiting indefinitely.

Budget for **all concurrent processes**, including old and new replicas during blue-green rollouts. Also reserve capacity for the scheduler's separate pool (up to five query connections plus its notification connections), migrations, other services, and operator access. Lower worker concurrency reduces demand but does not replace a connection budget. The coordination reserve supports the standard three listeners and shared leader connection; additional coordination consumers require revisiting that reserve. Session connections require a direct or session-pooled database endpoint, not transaction pooling.
