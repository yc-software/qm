...
AWS `aws.dbInstanceClass` and `aws.backupRetentionDays` select the RDS class and automated-backup retention rendered as `db_instance_class` and `backup_retention_days`; they default to `db.t4g.small` and 35 days. `aws.dbRetentionMinDays` is separate: it is the minimum retention that `qm up` accepts for its restore-point safety check and does not configure RDS. Before applying, the account must permit the chosen class and retention in the selected region and account plan; AWS availability and plan restrictions vary, and the rest of the deployment is not guaranteed to fit any free tier.
