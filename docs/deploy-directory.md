...
`qm update` checks that exact pin against npm without changing the deployment. `qm update --yes` installs the newest stable version outside the seven-day dependency cooldown and invokes its `up` command, so package metadata and runtime images advance together. `--version` accepts an exact release only after it has passed the same cooldown.

Fly and AWS scaffolds include `.github/workflows/qm-update.yml`. To enable browser updates, route a fine-grained GitHub token with Actions read/write access to the admin service as `QM_UPDATE_GITHUB_TOKEN`, and set `env.admin.QM_UPDATE_GITHUB_REPOSITORY` to the deployment repository's `owner/name`. `QM_UPDATE_GITHUB_REF` and `QM_UPDATE_GITHUB_WORKFLOW` default to `main` and `qm-update.yml`. The workflow uses GitHub OIDC for AWS; Fly deployments also set the repository secret `FLY_API_TOKEN`. The optional repository secret `QM_DEPLOY_ENV` restores the deployment's gitignored environment during the run. GitHub Actions must have permission to write repository contents so the exact package and lockfile pins can be safely recorded before deployment. Docker deployments require an operator-owned self-hosted equivalent and retain the command handoff by default.
