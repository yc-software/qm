...
Core stores share one query pool per database connection string and TLS configuration in each process. `PG_QUERY_POOL_MAX` caps that pool (default **8**); `PG_SESSION_POOL_MAX` separately caps long-held LISTEN and advisory-lock connections (default **8**). The query budget must be a positive integer; the session budget must be an integer of at least two. Up to four connections from the session budget are reserved for LISTEN and leader coordination, always leaving at least one for operation locks. With the defaults, this is four coordination connections plus four operation connections, not eight plus four. Increasing the session budget above eight adds operation capacity without growing the coordination reserve. These are non-secret core environment settings. Closing a store releases its ownership; the last owner closes the underlying pool. Acquiring a connection times out after five seconds rather than waiting indefinitely.
