...
`env.core.DIRECTORY_SERVICE_PRINCIPALS` accepts a JSON array of service identities with `principalId`, `displayName`, and optional `slackId`. QM overlays these identities onto the people directory without persisting them as surface-owned members, preserves them across surface directory replacement, and removes them when configuration is removed. They remain subject to explicit principal deactivation.
