...
test("the composer query parameter selects a variant and persists it", async () => {
  ...
  const mod = await load("http://localhost/s/abc?composer=t3", storage);
  assert.equal(mod.composerVariant(), "t3");
  assert.equal(storage.data.get("web-ui:composer-variant"), "t3");
});

test("an unknown variant falls back to the stored one, then to the default", async () => {
  const storage = fakeStorage();
  storage.data.set("web-ui:composer-variant", "claude");
  const stored = await load("http://localhost/?composer=nope", storage);
  assert.equal(stored.composerVariant(), "claude");
...
test("setting the default clears the stored preference", async () => {
  const storage = fakeStorage();
  const mod = await load("http://localhost/?composer=chatgpt", storage);
