...
test("a tab carries only its close control; sharing and archive live in the group header", () => {
...
test("every header orders tools, sharing and archive before the pane controls, which fold when narrow", () => {
  ...
  assert.match(
    render,
    /split-tools[\s\S]*?sessionActions\(sessionId, panel!\.id\)[\s\S]*?split-pane-actions-wide[\s\S]*?buttons\.filter\(\(b\) => !b\.cls\)/,
  );
  ...
  assert.doesNotMatch(css, /\.split-group-session-action\s*\{\s*display: none/);
