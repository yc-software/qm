...
const core = createServer((req: IncomingMessage, res) => {
  ...
  if (new URL(u, "http://core.test").pathname === "/d/redirecting-app/") {
    res.writeHead(302, { location: "/d/redirecting-app/home" });
    return void res.end();
  }
  if (new URL(u, "http://core.test").pathname === "/d/forbidden-app/") {
    res.writeHead(403, { "content-type": "application/json" });
    return void res.end(JSON.stringify({ error: "forbidden" }));
  }
  if (u.startsWith("/d/app1/assets/file.bin?v=2")) {
...
function assertAuthenticatedRequest(index: number, url: string): void {
  ...
  assert.equal(request.url.replace(/([?&])_sourceAuthNonce=[^&]*/g, ""), url);
...
for (const prefix of ["/deployments", "/d"]) {
  test(`GET ${prefix}/:id/ relays a gzip-serving app as a decodable response`, async () => {
    ...
    assert.equal(r.headers["content-encoding"], "gzip", "streamed compressed bytes retain their encoding");
    ...
    assert.equal(gunzipSync(r.body).toString(), PAGE, "the body reaches the browser decodable as declared");
    assert.equal(
  ...
  for (const method of ["POST", "DELETE"]) {
    test(`${method} ${prefix}/:id/ remains outside the launch proxy`, async () => {
      const before = coreRequests.length;
