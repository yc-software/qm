...
const deliveredFile = readFileSync(new URL("../src/delivered-file.ts", import.meta.url), "utf8");
...
test("SVG attachments fall back to a visible download chip", () => {
  ...
  assert.match(deliveredFile, /if \(!browserRenderableImage\(file\.mimetype\)\) return imageChip/);
});
