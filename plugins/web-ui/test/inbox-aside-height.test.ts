...
function heightFor(viewport: number): number {
  return Math.min(MAX, Math.max(MIN, viewport - PAD_TOP - GAP));
}

test("the assistant tracks the viewport between its bounds, so the composer rides the bottom", () => {
  assert.equal(heightFor(900), 864);
  assert.equal(heightFor(700), 664);
  assert.equal(heightFor(500), 464);
});
...
test("a very short window overrides the fit: the assistant keeps its floor and overflows into page scroll", () => {
  ...
  assert.ok(MIN > 300 - PAD_TOP - GAP, "the floor must exceed what a short viewport offers, or nothing overflows");
});
...
test("sizeAside clamps on both ends rather than only flooring", () => {
  ...
  assert.match(sizeAside, /host\.clientHeight - padTop - ASIDE_BOTTOM_GAP/);
  assert.match(inbox, new RegExp(`const ASIDE_MIN_HEIGHT = ${MIN};`));
  assert.match(inbox, new RegExp(`const ASIDE_MAX_HEIGHT = ${MAX};`));
  assert.match(inbox, new RegExp(`const ASIDE_BOTTOM_GAP = ${GAP};`));
});
