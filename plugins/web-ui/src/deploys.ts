...
function versionLabel(d: DeploymentView): string {
  ...
  if (d.appliedVersion !== undefined && d.appliedVersion !== d.currentVersion)
    return t("v{live} live · v{pending} pending", { live: d.appliedVersion, pending: d.currentVersion });
  ...
  return html`
    <div class="session-menu deploy-menu">
      <button
        ...
        type="button"
        aria-label=${t("More actions for {name}", { name: deploymentTitle(d) })}
        aria-haspopup="menu"
...
async function openDeploy(d: DeploymentView): Promise<void> {
  ...
  try {
    ...
    deployNotices = withDeploymentDetailNotice(
      deployNotices,
      d.id,
      errMessage(error, t("Could not load app details.")),
    );
    drawDeployDetail(d);
  }
}
        ...
        <section class="deploy-detail-section">
          ...
          <div class="deploy-facts">
            ...
            <div>
              ...
              ><strong
                >${deploymentLatestAt(d) ? new Date(deploymentLatestAt(d)).toLocaleString(currentLocale()) : "—"}</strong
              >
            </div>
            <div>
        ...
        ${
          canManage(d)
            ? html`<section class="deploy-detail-section">
                ...
                <div class="deploy-setting-row">
                  <div>
                    <strong>${t("URL slug")}</strong
                    ><span>${t("Changes the app URL. Existing links do not redirect.")}</span>
                  </div>
                  ...
                  ${versions.map(
                    (version) => html`
                      <div class="deploy-version-row">
                        ...
                        <div>
                          <span>${new Date(version.createdAt).toLocaleString(currentLocale())}</span
...
async function commitEditDeploy(d: DeploymentView): Promise<void> {
  ...
  try {
    ...
    if (currentDeployActionView(d.id) === "target") {
      ...
      deployToast = { deployment: updated, text: t("{name} settings saved.", { name: deploymentTitle(updated) }) };
    ...
    >
      ...
      >
        <div class="project-dialog-head">
          <div><h2 id="deploy-archive-title">${t("Archive {name}?", { name: deploymentTitle(d) })}</h2></div>
        </div>
        <p>
...
async function archiveDeploy(d: DeploymentView): Promise<void> {
  ...
  try {
    ...
    deployToast = {
      ...
      text: t("{name} is offline and archived.", { name: deploymentTitle(d) }),
      undo: true,
    };
    await refreshDeployments();
...
async function restoreDeploy(d: DeploymentView): Promise<void> {
  ...
  try {
    ...
    deployToast = {
      deployment: restoredResponse,
      text: t("{name} is restored and running.", { name: deploymentTitle(d) }),
    };
...
async function openLiveEdit(d: DeploymentView, button: HTMLButtonElement): Promise<void> {
  ...
  try {
    ...
    if (!r.url) throw new Error(t("No live URL is available for this app."));
    ...
    deployNotices = withDeploymentDetailNotice(
      deployNotices,
      d.id,
      errMessage(error, t("Could not open live editing.")),
    );
