        ...
        <div class="kc-resource-copy">
          ...
          <div class="kc-credential-facts">
            <div class="kc-audit-line">
              ${icon(Activity, 14)}${lastUse ? html`${t("Last used {date} in {scope}", { date: fmtDate(lastUse.ts), scope: scopeName(lastUse.scopeLabel) })} · ${t(lastUse.status)}` : t("No audited use yet")}
            </div>
              ...
              ${asks.map(
                (ask) =>
                  html`<div class="kc-access-row">
                    <div>
                      <strong>${scopeName(ask.requesterScopeId)}</strong>
                      ${t("requested {mode} access", { mode: t(ask.requestedMode ?? "one-time") })}
                ...
                (grant) =>
                  html` <div class="kc-access-row">
                    <div>
                      ...
                      <div>
                        ${grant.purpose}${grant.expiresAt ? ` · ${t("expires {date}", { date: fmtDate(grant.expiresAt) })}` : ""}
                      </div>
                    </div>
                    <button
...
function scopeName(scope: string): string {
  ...
  return `${t(kind ? kind.charAt(0).toUpperCase() + kind.slice(1) : "Context")}: ${name}`;
}
      ...
      secureDropUrl
        ? html`
            <div class="kc-success" role="status">
              <strong>${t("Secure form ready")}</strong
              ><span>${t("Open it in a new tab to enter the credential.")}</span>
            </div>
            <div class="kc-form-actions">
              <a class="btn primary" href=${secureDropUrl} target="_blank" rel="noopener noreferrer"
                >${t("Open secure form")}</a
              ><button
                class="btn"
                type="button"
                @click=${() => {
        ...
        : html`
            ...
            <div class="kc-form-actions">
              ...
              >
                ${t(keychainOperations.dropInFlight ? "Creating…" : "Create secure form")}
              </button>
            </div>
          `
    }
...
export function noteConnectorResult(provider: string, status: string): void {
  ...
  connectorNotice =
    status === "connected" ? t("{name}: connected.", { name }) : t("{name}: connection failed.", { name });
}
          ...
          <div class="kc-hero-copy">
            ...
            <div class="kc-trust-note">
              ${icon(ShieldCheck, 14)}<span
                >${t("Secrets stay encrypted and every use or shared grant is audited.")}</span
              >
            </div>
          </div>
            ...
            ${
              keychainCredentials.length
                ...
                : html`<div class="kc-empty">
                    ${icon(KeyRound, 20)}
                    <div>
                      <strong>${t("No stored credentials")}</strong
                      ><span>${t("Add one without pasting a secret into chat.")}</span>
                    </div>
                    <button
                      class="btn"
                      type="button"
...
async function deleteCredential(credential: KeychainCredential): Promise<void> {
  ...
  const impact = active.length
    ? t(
        ...
        {
          count: active.length,
          scopes: active.map((grant) => scopeName(grant.audienceScopeId)).join(", "),
        },
      )
    : "";
  ...
  confirmation = {
    ...
    body: `${impact}${impact ? " " : ""}${t("Automations using it may stop working. The credential cannot be recovered.")}`,
...
async function revokeGrant(grant: KeychainGrant): Promise<void> {
  ...
  confirmation = {
    ...
    body: t("This {mode} access ends immediately. Automations using it may stop working.", {
      mode: t(grant.mode === "standing" ? "standing" : "one-time"),
    }),
...
async function revokeConnector(provider: string): Promise<void> {
  ...
  confirmationOpener = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  confirmation = {
    title: t("Disconnect {name}?", { name: CONNECTOR_LABELS[provider]?.name ?? provider }),
    body: `${impact}${impact ? " " : ""}${t("Automations using this account may stop working.")}`,
