...
function recentItem(item: RecentItem): TemplateResult {
  ...
  if (item.groupKind === "personal") fallbackName = t("Personal");
  else if (item.groupKind === "channel") fallbackName = t("Channel");
  else if (item.groupKind === "group") fallbackName = t("Group DM");
  const name = item.groupKind === "personal" ? fallbackName : (item.name ?? fallbackName);
        ...
        s.id
          ? html`<span class="chat-row-actions">
              <button
                ...
                type="button"
                title=${t("Copy link")}
                aria-label=${t("Copy link to {title}", { title: sessionTitle(s) })}
              ...
              <button
                ...
                aria-label=${s.pinned ? t("Unpin {title}", { title: sessionTitle(s) }) : t("Pin {title}", { title: sessionTitle(s) })}
                @click=${() => {
              ...
              <button
                ...
                aria-label=${s.archived ? t("Unarchive {title}", { title: sessionTitle(s) }) : t("Archive {title}", { title: sessionTitle(s) })}
                @click=${() => {
...
function groupedRows(list: RecentItem[]): TemplateResult {
  ...
  for (const item of list) {
    ...
    if (!dateless && g !== group) {
      group = g;
      items.push({ key: `group:${g}`, tpl: html`<div class="recents-group">${recencyGroupLabel(g)}</div>` });
    }
...
function recencyGroupLabel(group: string): string {
  if (group === "Today") return t("Today");
  if (group === "Yesterday") return t("Yesterday");
  if (group === "Previous 7 days") return t("Previous 7 days");
  if (group === "Previous 30 days") return t("Previous 30 days");
  if (group === "Older") return t("Older");
  return group;
}
