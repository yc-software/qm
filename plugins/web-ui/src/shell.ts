...
import { activeLocale, advertiseLocales, availableLocales, initI18n, t, type Locale } from "./i18n.ts";
...
function impersonationBanner(by: string) {
  return html`
    <div class="top-banner" role="status">
      <span>${t("Viewing the assistant as {user} — you are {by}", { user: appState.me?.user ?? "", by })}</span>
...
function devBanner(user: string) {
  return html`
    <div class="top-banner dev" role="status">
      <span>${t("Dev mode — no identity provider, signed in as {user}", { user })}</span>
...
function deniedGate() {
  return gateShell(html`
    ...
    ${authMode === "dev" ? html`<div class="hint">${t("This instance lists its principals in WEB_UI_PRINCIPALS.")}</div>` : nothing}
  `);
}
...
function sidebarFooterTpl(): TemplateResult {
  ...
  return html`
    ...
    <a class="icon-btn subtle" href=${ADMIN_HOME_URL} title=${t("Back to admin")} aria-label=${t("Back to admin")}
      >${icon(ArrowLeft, 17)}</a
    >
    <div class="menu-control lang-control">
      <button
        ...
        @click=${(e: Event) => toggleLangMenu(e)}
      >
        ${icon(Languages, 17)}
      </button>
      ${langMenuOpen
        ? html`
            <div class="menu-popover" role="menu" @click=${(e: Event) => e.stopPropagation()}>
              <div class="menu-title">${t("Language")}</div>
              ${availableLocales().map(
                (option) => html`
                  <button
                    class="menu-option ${option.id === current ? "active" : ""}"
                    type="button"
                    role="menuitemradio"
                    aria-checked=${option.id === current ? "true" : "false"}
                    @click=${() => void selectLocale(option.id)}
                  >
                    <span class="menu-option-copy">
                      <span class="menu-option-label">${option.name}</span>
                    </span>
                    ${option.id === current ? icon(Check, 15) : nothing}
                  </button>
                `,
              )}
            </div>
...
function renderSidebarFooter(): void {
  const footer = (appEl as HTMLElement).querySelector("#sidebar-footer");
  if (footer) render(sidebarFooterTpl(), footer as HTMLElement);
}
...
export function closeLangMenu(target?: Element | null): boolean {
  if (!langMenuOpen) return false;
  if (target?.closest(".lang-control")) return false;
  langMenuOpen = false;
  renderSidebarFooter();
  return true;
}
...
async function selectLocale(locale: Locale): Promise<void> {
  ...
  langMenuOpen = false;
  try {
    await updateLocalePreference(`personal:${appState.me?.user}`, locale);
  } catch (e) {
    swallow("web-ui: persist locale preference", e);
    renderSidebarFooter();
    return;
  }
  location.reload();
    ...
    html`
      ...
      <div class="layout ${sidebarOpen ? "" : "sidebar-closed"} ${banner ? "bannered" : ""}">
        <aside class="sidebar" aria-label=${t("Navigation")} @keydown=${onSidebarKeydown}>
          ...
          <div class="sidebar-footer" id="sidebar-footer">${sidebarFooterTpl()}</div>
        </aside>
...
export function renderSidebarTop(): void {
  ...
  const navGroup = (id: string, title: string, open: boolean, toggle: () => void, rows: TemplateResult) => html`
    <button
      ...
      aria-controls=${id}
      title=${open ? t("Hide {title}", { title }) : t("Show {title}", { title })}
      @click=${toggle}
    >
      <span>${title}</span>
...
export async function boot(): Promise<void> {
  ...
  const [runtimeConfig, surfaceConfig, localePref] = await Promise.all([
    fetchRuntimeConfig(personalScope),
    fetchSurfaceConfig(),
    fetchLocalePreference(personalScope),
  ]);
  advertiseLocales(surfaceConfig?.locales);
  await initI18n({ stored: localePref?.locale, orgDefault: localePref?.orgLocale ?? surfaceConfig?.orgLocale });
  if (runtimeConfig)
    applyRuntimeOptions(
  ...
  if (embedMode) {
    if (wantedSession) {
      ...
      else showMainEmpty(t("That conversation wasn't found, or you don't have access to it."));
    } else {
