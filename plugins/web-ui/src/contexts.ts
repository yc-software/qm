...
function contextMeta(c: CoreContext): { title: string; sub: string; glyph: IconNode } {
  if (c.project) {
    ...
    return {
      ...
      sub:
        memberCount === 1
          ? t("{count} member", { count: memberCount })
...
function gridTpl(): TemplateResult {
  const status = contextsNotice || (contextsLoading && contextsState.list.length === 0 ? t("Loading projects…") : "");
...
function contextCard(c: CoreContext): TemplateResult {
  ...
  const count = c.sessionCount === 1 ? t("1 conversation") : t("{count} conversations", { count: c.sessionCount });
  let access = t("shared");
  if (c.project && isProjectOwner(c)) access = t("owned");
  else if (c.kind === "personal") access = t("private");
  return html`
    <button class="card context-card" type="button" @click=${() => selectContext(c.scopeId)}>
      ...
      <div class="card-meta">${c.lastActivityAt ? t("{count} · active {time}", { count, time: relTime(c.lastActivityAt) }) : count}</div>
    </button>
  `;
}
  ...
  return html`
    <div class="context-detail">
      ...
      <div class="context-detail-head">
        ...
        <div class="context-detail-titles">
          ...
          <div class="context-sub">
            ${c.project ? sub : `${sub} ${t("The agent's files and memory here are separate from your other contexts.")}`}
          </div>
        </div>
      ...
      <div class=${`context-workspace ${hasSettings ? "has-settings" : ""}`}>
            ...
            completelyEmpty
              ? html`
                  <section class="context-panel context-project-empty">
                    ...
                    <p>
                      ${t("Start a conversation with New chat. Files, automations, and other work created there will stay scoped to this project.")}
                    </p>
                  </section>
                `
              : html`
        ...
        ${
          hasSettings
            ? html`<aside class="context-settings" aria-label=${c.project ? t("Project settings") : t("Context settings")}>
...
function memberPicker(context: CoreContext): TemplateResult {
  ...
  if (idle && contextsState.memberSearchedQuery && matches.length === 0) {
    emptyNote = contextsState.memberMatches.length
      ...
      : t("No matches for “{query}”.", { query: contextsState.memberSearchedQuery });
  }
...
function resourceSections(scopeId: string): TemplateResult | typeof nothing {
  ...
  return html`
    ${r.files.length ? resourceGroup("files", t("Files"), r.files.map(fileRow)) : nothing}
    ${
      r.skills.length
        ? resourceGroup(
            "skills",
    ...
    ${r.deployments.length ? resourceGroup("deploys", t("Apps"), r.deployments.map(deploymentRow)) : nothing}
  `;
}
...
function cronRow(c: CronView, manage = false): TemplateResult {
  ...
  if (c.archived) status = t("archived");
  else if (c.enabled) status = t("enabled");
  ...
  return html`
    <div class="context-session-row context-resource-row">
      ...
      <span class="context-session-meta">
        ...
        <span class="badge">${d.permission === "write" ? t("manage") : t("read")}</span>
      </span>
    </div>
  `;
}
...
async function createProject(event: SubmitEvent): Promise<void> {
  ...
  try {
    ...
    if (!project) throw new Error(t("Core returned an invalid project"));
    const loaded = contextsState.loaded;
...
async function removeProjectMember(context: CoreContext, principalId: string): Promise<void> {
  if (!context.project || contextsState.memberBusy) return;
  const label = memberLabel(context, principalId);
  if (!window.confirm(t("Remove {label} from {project}?", { label, project: context.name || t("this project") }))) return;
  const resetSeq = contextsResetSeq;
  memberSearchSeq++;
  cancelMemberSearchTimer();
  contextsState.memberSearching = false;
