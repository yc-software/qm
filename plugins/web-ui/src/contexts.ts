...
function contextMeta(c: CoreContext): { title: string; sub: string; glyph: IconNode } {
  if (c.project) {
    ...
    return {
      ...
      sub: t(memberCount === 1 ? "{count} member" : "{count} members", { count: memberCount }),
      glyph: Folder,
    };
  }
  if (c.kind === "personal") {
...
export function scopeChip(scopeId: string | null, fallbackName?: string | null): TemplateResult {
  ...
  return html`<span class="scope-chip" title=${t("In {name}", { name: title })}
  ...
  return html`
    <div class="menu-control form-menu-control scope-filter">
      <button class="menu-button" type="button" aria-haspopup="menu" aria-expanded="false" @click=${toggleFormMenu}>
        ${icon(ListFilter, 14)}<span class="menu-label">${t("Filter by: {name}", { name: label })}</span
        >${icon(ChevronDown, 14)}
      </button>
...
function contextCard(c: CoreContext): TemplateResult {
  ...
  const count = t(c.sessionCount === 1 ? "1 conversation" : "{count} conversations", { count: c.sessionCount });
  let access = "shared";
  ...
  return html`
    <button class="card context-card" type="button" @click=${() => selectContext(c.scopeId)}>
      ...
      <div class="card-meta">
        ${count}${c.lastActivityAt ? ` · ${t("active {time}", { time: relTime(c.lastActivityAt) })}` : ""}
      </div>
    </button>
  `;
}
  ...
  return html`
    <div class="context-detail">
      ...
      <div class=${`context-workspace ${hasSettings ? "has-settings" : ""}`}>
        ...
        ${
          hasSettings
            ? html`<aside class="context-settings" aria-label=${t(c.project ? "Project settings" : "Context settings")}>
  ...
  return html`
    <div class="context-session-row context-resource-row">
      ...
      <span class="context-session-meta">
        ...
        <span class="badge">${t(d.permission === "write" ? "manage" : "read")}</span>
      </span>
    </div>
  `;
}
      ...
      <form @submit=${(event: SubmitEvent) => void createProject(event)}>
        ...
        <div class="project-dialog-actions">
          <button class="btn" type="button" @click=${closeCreateProject}>
            ${t(contextsState.createSaving ? "Close" : "Cancel")}
          </button>
          <button class="btn primary" type="submit" ?disabled=${contextsState.createSaving}>
            ${icon(FolderPlus, 15)}<span>${t(contextsState.createSaving ? "Creating…" : "Create project")}</span>
          </button>
        </div>
      </form>
    </dialog>
...
async function removeProjectMember(context: CoreContext, principalId: string): Promise<void> {
  ...
  const label = memberLabel(context, principalId);
  if (
    !window.confirm(t("Remove {person} from {project}?", { person: label, project: context.name || t("this project") }))
  )
    return;
  const resetSeq = contextsResetSeq;
  memberSearchSeq++;
