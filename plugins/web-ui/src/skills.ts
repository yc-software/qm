...
function skillMeta(s: SkillItem): string {
  const source = s.source === "pack" ? t("Pack {name}", { name: s.pack?.upstreamName ?? "source" }) : t("Created here");
    ...
    <div class="skill-variant ${archived ? "archived" : ""}">
      ...
      <div class="skill-variant-copy">
        ...
        <div class="skill-variant-meta">
          ${skillMeta(s)}${s.assetCount ? ` · ${s.assetCount === 1 ? t("{count} asset", { count: s.assetCount }) : t("{count} assets", { count: s.assetCount })}` : ""}
        </div>
  ...
  return html`<section class="skill-group">
    <div class="skill-group-head">
      <h2 class="skill-group-name">
        <code>/${name}</code>${skills.length > 1 ? html`<span>${t("{count} variants", { count: skills.length })}</span>` : nothing}
      </h2>
  ...
  if (!e) {
    return html`<section class="skill-form-page">
      ...
      <div class="skill-form-heading">
        <div>
          <h1 class="pane-title">${t("Edit /{name}", { name: editingTarget?.name ?? "skill" })}</h1>
          <p>${editError ? t("Instructions unavailable.") : t("Loading instructions…")}</p>
        </div>
      </div>
    ...
    >
      ...
      <div class="skill-form-heading">
        <div>
          ...
          <p>${t("Available to {audience}", { audience: e.scopeId?.startsWith("personal:") ? t("only you") : (e.scopeId ?? t("this context")) })}</p>
        </div>
      ...
      ${
        reviewed
          ? html`<div class="skill-impact" role="alert">
              ...
              <div class="card-meta">
                ${t("Everyone in this context can invoke the updated instructions. Description {descriptionStatus}; instructions {bodyStatus}.", {
                  descriptionStatus: e.description === e.originalDescription ? t("unchanged") : t("changed"),
                  bodyStatus: e.body === e.originalBody ? t("unchanged") : t("changed"),
                })}
              </div>
            </div>`
          : nothing
      ...
      filters: html`<div class="skill-registry-controls">
        ...
        <div class="skill-result-count" aria-live="polite">
          ${loading
            ? t("Loading…")
            : t("{skills} in {groups}", {
                skills: filtered.length === 1 ? t("{count} skill", { count: filtered.length }) : t("{count} skills", { count: filtered.length }),
                groups: groups.length === 1 ? t("{count} group", { count: groups.length }) : t("{count} groups", { count: groups.length }),
              })}
        </div>
...
function archiveDialog(skill: SkillItem): TemplateResult {
  const audience = skill.scope === "personal" ? t("you") : t("everyone in {scope}", { scope: skill.scopeId ?? `this ${skill.scope}` });
  return html`<div
  ...
  >
    ...
    >
      ...
      <p id="skill-archive-impact">
        ${t("This version will stop being available to {audience}. If it overrides a broader /{name}, that version becomes effective. Its history and assets are kept, and you can restore it later.", { audience, name: skill.name })}
      </p>
      <div class="project-dialog-actions actions">
        ...
        >
          ${deleting === skill.id ? t("Archiving…") : t("Archive skill")}
        </button>
      </div>
    </div>
  </div>`;
