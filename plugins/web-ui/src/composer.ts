...
export function composerForm(agent: Agent): TemplateResult {
  ...
  if (composerState.processingFiles) {
    ...
    composerNotice = html`<div class="composer-error">
      ${composerState.error || t("Loading runtime settings…")}
      ${composerState.error ? html`<button type="button" @click=${() => void refreshRuntimeSelection(chatState.scopeId, agent)}>${t("Retry")}</button>` : nothing}
    </div>`;
          ...
          ? html`<div class="runtime-upgrade">
              <span
                >${t("The org now recommends {harness} · {model}.", {
                  harness: modelOptionFor(`${activeRuntimeConfig.orgDefault.harnessId}:${activeRuntimeConfig.orgDefault.modelId}`).harnessLabel,
                  model: modelOptionFor(`${activeRuntimeConfig.orgDefault.harnessId}:${activeRuntimeConfig.orgDefault.modelId}`).buttonLabel,
                })}</span
              >
              <button
                type="button"
                ...
                ${composerState.attachments.map(
                  (a) => html`
                    <span class="file-chip">
                      ...
                      <button type="button" class="chip-x" title=${t("Remove")} @click=${() => removeAttachment(a.id, agent)}>
                        ${icon(X, 13)}
                      </button>
                    </span>
                  `,
...
function sendControls(agent: Agent): TemplateResult {
  ...
  return html`
    ...
    <button
      class="send-btn"
      type="submit"
      title=${t("Steer the running task")}
      aria-label=${t("Steer the running task")}
      ?disabled=${!canSteer}
    >
      ${icon(ArrowUp, 17)}
    </button>
...
function settingsControl(agent: Agent, selected: ModelOption, disabled: boolean): TemplateResult {
  ...
  const fastOn = fastAvailable && effectiveFastMode();
  const summary = `${selected.buttonLabel} · ${effortLabel(composerState.effortLevel)}${fastOn ? ` · ${t("Fast")}` : ""}`;
  return html`
...
async function addFiles(files: File[], agent: Agent, folders: DropEntryLike[] = []): Promise<void> {
  ...
  try {
    ...
    if (err instanceof FolderDropError) composerState.error = err.message;
      ...
      composerState.error = t("That drop included a folder this browser can't read — zip it and drop the archive instead.");
