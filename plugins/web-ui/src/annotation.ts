...
class AnnotationDirective extends AsyncDirective {
  ...
  private install() {
    ...
    const show = (event: Event) => {
      ...
      let quote: string;
      let anchor: () => DOMRect;
      let focusTarget: HTMLElement = root;
      ...
      const cancelEditing = () => {
        ...
        if (!focusTarget.hasAttribute("tabindex") && focusTarget === root) focusTarget.tabIndex = -1;
      ...
      const position = () => {
        ...
        popup.style.top = `${Math.max(8, Math.min(rect.bottom + 8, window.innerHeight - bounds.height - 8))}px`;
      };
      const outside = (event: PointerEvent) => {
        if (!this.editing && !popup.contains(event.target as Node)) this.close();
      };
      const escape = (event: KeyboardEvent) => {
        if (event.key === "Escape" && (!this.editing || popup.contains(document.activeElement))) {
      ...
      const observer = new MutationObserver(() => {
        if (!root.isConnected) this.close();
      });
