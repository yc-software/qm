...
function drawCronsPage(): void {
  ...
  if (cronTab === "yours") {
    ...
    if (all.length && !yoursEnabled.length)
      rows.push(cronEmptyRow(t(ownsAny ? "No active crons." : "None of your own crons yet.")));
...
function openCron(c: CronView): void {
  ...
  let stateActions = html`
    <button class="btn" @click=${() => void setCronEnabled(c.id, true)}>
      ${icon(Play, 15)}<span>${t("Enable")}</span>
    </button>
    ...
    html`
      <div class="resource-detail">
        ...
        <div class="field">
          ...
          <div class="value">${next != null ? new Date(next).toLocaleString(currentLocale()) : "—"}</div>
        </div>
        <div class="field">
          <label>${t("Last fired")}</label>
          <div class="value">
            ${c.lastFiredAt ? new Date(c.lastFiredAt).toLocaleString(currentLocale()) : t("Never")}
          </div>
        </div>
        ...
        ${
          manageable
            ? html`
              ...
              `
            : html`<div class="hint">
                ${t("Shared from {scope} — you can view it, but not change it.", { scope: cronScopeLabel(c) })}
              </div>`
        }
...
function cronRunHistory(c: CronView): TemplateResult {
  ...
  const heading = html`<div class="cron-run-heading">
    <label>${t("Recent runs")}</label
    ><button class="btn" type="button" @click=${() => refreshCronRuns(c.id)}>${t("Refresh")}</button>
  </div>`;
  if (!runs)
        ...
        (run) =>
          html` <div class="cron-run-row">
            <div>
              ...
              <span>${new Date(run.firedAt).toLocaleString(currentLocale())}</span>
            </div>
...
function cronDialogTpl(dialog: { kind: "rename" | "delete"; cron: CronView }): TemplateResult {
    ...
    >
      <div class="project-dialog cron-edit-dialog" role="dialog" aria-modal="true" aria-labelledby="cron-delete-title">
        <div class="project-dialog-head">
          <div><h2 id="cron-delete-title">${t("Delete {name}?", { name: cronTitle(c) })}</h2></div>
        </div>
        <p>
  ...
  >
    ...
    >
      ...
      <p class="hint">
        ${t(c.message === undefined ? "To change the schedule, timezone, destination, or run mode, use the agent so it can validate the resulting behavior and permissions." : "To change the message, schedule, timezone, destination, or run mode, use the agent so it can validate the resulting behavior and permissions.")}
      </p>
...
function editCronWithAgent(c: CronView): void {
  ...
  void chatState.agent?.prompt(
    t(
      ...
      { id: c.id, title: cronTitle(c), schedule: cronScheduleSummary(c) },
    ),
  );
}

async function archiveCron(id: string, archived: boolean): Promise<void> {
  const ok = await patchCron(id, { archived }, t(archived ? "archive failed" : "unarchive failed"));
  if (!ok) return;
  await refreshCrons();
  if (archived) {
    cronTab = "yours";
...
function setCronEnabled(id: string, enabled: boolean): Promise<void> {
  return cronMutate(async () => {
    try {
      ...
      cronActionNotice = errMessage(e, t(enabled ? "enable failed" : "disable failed"));
    }
    await reopenCron(id);
  }, undefined);
}
