...
export function themeTokens(palette: Palette): ThemeTokens {
  ...
  const vars: Record<string, Rgb> = {
    ...
    "--inset": step(0.03, 0.015),
    "--hover": step(0.07, 0.03),
    "--hover-2": step(0.11, 0.06),
    "--ink": fg,
    "--ink-2": mutedForeground,
    "--ink-3": mix(fg, bg, 0.55),
    "--line": border,
    "--line-strong": step(0.17, 0.14),
    "--line-soft": step(0.08, 0.06),
    "--field": step(0.09, 0.04),
    "--stripe": mix(bg, fg, 0.075),
    "--stripe-bg": step(0.02, 0.015),
    "--blue": action,
    "--blue-ink": link,
    ...
    "--red": destructive,
    "--green-ink": mix(success, fg, 0.35),
    "--orange-ink": mix(warning, fg, 0.35),
    "--red-ink": mix(destructive, fg, 0.35),
    "--red-tint": mix(bg, destructive, 0.14),
    "--tooltip-bg": dark ? mix(bg, { r: 0, g: 0, b: 0 }, 0.25) : mix(fg, bg, 0.1),
    "--tooltip-fg": dark ? fg : bg,
