...
| `smtp`    | `SMTP_HOST`, and optionally `SMTP_USERNAME` with `SMTP_PASSWORD`, `SMTP_PORT`, `SMTP_TLS` | Any relay. Leave both credentials unset for a relay that authorizes by source IP; setting only one is refused. `SMTP_PORT` defaults to `587`. `SMTP_TLS` defaults to `implicit` on port `465` and `starttls` otherwise; `none` is refused in production, and a relay that does not advertise STARTTLS is refused rather than sent credentials in cleartext. |
