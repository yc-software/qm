...
import { readPasswordHashes } from "./passwords.ts";
...
export function readConfig(env: NodeJS.ProcessEnv): AuthConfig {
  ...
  return {
    ...
    loginMethod: env.AUTH_LOGIN_METHOD?.trim() || "email",
    passwordHashes: readPasswordHashes(env.AUTH_PASSWORD_HASHES),
...
export function bootProblems(cfg: AuthConfig, isProd: boolean): string[] {
  ...
  if (cfg.loginMethod !== "email" && cfg.loginMethod !== "password") {
    problems.push("AUTH_LOGIN_METHOD must be email or password");
  }
  if (cfg.loginMethod === "password") {
    if (!cfg.passwordHashes || [...cfg.passwordHashes.keys()].some((email) => !validEmail(email))) {
      problems.push(
        "AUTH_PASSWORD_HASHES must contain email addresses mapped to scrypt hashes generated by qm password",
      );
    }
  } else {
