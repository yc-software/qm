...
let job: Record<string, unknown> = {
  ...
  targetVersion: "0.2.0",
  state: "running",
  runUrl: "https://github.com/acme/qm-deployment/actions/runs/5",
  createdAt: Date.now(),
  updatedAt: Date.now(),
};
...
const core = createServer(async (req: IncomingMessage, res) => {
  ...
  if (req.method === "PATCH" && req.url === "/v1/admin/updates/orphan-job-1") {
    const chunks: Buffer[] = [];
...
process.env.CORE_SIGNING_SECRET = "admin-update-unconfigured-test-secret";
process.env.NODE_ENV = "test";
process.env.ALLOW_UNSIGNED_TEST_IDENTITY = "1";
process.env.QM_VERSION = "0.1.9";
delete process.env.QM_UPDATE_GITHUB_REPOSITORY;
delete process.env.QM_UPDATE_GITHUB_TOKEN;
...
test("an open job is released when the browser updater is no longer configured", async () => {
  const response = await originalFetch(`${base}/api/update`, { headers: { cookie: "admin=U-admin" } });
  assert.equal(response.status, 200);
  const body = (await response.json()) as { updater: { available: boolean; job: { state: string; detail: string } } };
  assert.equal(body.updater.available, false);
  assert.equal(body.updater.job.state, "failed");
  assert.equal(body.updater.job.detail, "Browser updater is no longer configured");
});
